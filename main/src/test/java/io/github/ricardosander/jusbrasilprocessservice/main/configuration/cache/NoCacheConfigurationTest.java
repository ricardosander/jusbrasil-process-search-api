...
@SpringBootTest(properties = { "cache.local.enabled=false", "cache.cloud.enabled=false"})
@ComponentScan(basePackageClasses = ProcessSearchApplication.class)
class NoCacheConfigurationTest {
    ...
    void shouldLoadLocalCacheRetrieveProcessGateway_whenLocalCacheConfigIsEnabled() {
        assertThat(processGateway)
                .isNotNull()
                .isInstanceOf(RetrieveProcessGateway.class)
                .isNotInstanceOf(CloudCacheRetrieveProcessGateway.class)
                .isNotInstanceOf(LocalCacheRetrieveProcessGateway.class);
    }
}
